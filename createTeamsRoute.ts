...
async function fetchDrivers(client: any): Promise<any[]> {
...
export async function createTeamsHandler(req: Request, res: Response) {
  try {
...
function generateRandomTeamsData(driverIds: number[], count: number, drivers: any[]): any[] {
  ...
  for (let i = 0; i < count; i++) {
    const name = casual.last_name + ' ' + generateRandomWord(['Racing', 'Team', 'Motorsport']);
...
function generateRandomWord(words: string[]): string {
